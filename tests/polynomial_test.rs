...
fn test_polynomial_addition_different_sizes() {
    let p1 = Polynomial::new(vec![fp!(1u64)]);
    let p2 = Polynomial::new(vec![fp!(2u64), fp!(3u64), fp!(4u64)]);
...
fn test_polynomial_multiplication_with_zero() {
    let zero_poly = Polynomial::<DefaultFieldConfig>::zero();
...
fn test_polynomial_evaluation() {
    // P(x) = 2 + 3x + 4x^2
    let poly = Polynomial::new(vec![fp!(2u64), fp!(3u64), fp!(4u64)]);
...
fn test_polynomial_evaluation_at_zero() {
    let poly = Polynomial::new(vec![fp!(42u64), fp!(10u64), fp!(5u64)]);
    let x = fp!(0u64);
...
fn test_polynomial_trim() {
    ...
    let poly = Polynomial::new(vec![fp!(1u64), fp!(2u64), fp!(0u64), fp!(0u64)]);
...
fn test_polynomial_debug_format() {
    let poly = Polynomial::new(vec![fp!(1u64), fp!(2u64)]);
