...
fn main() {
    ...
    // P(x) = 5x^4 + 3x^2 + 7
    let p2 = Polynomial::new(vec![fp!(7u64), fp!(0u64), fp!(3u64), fp!(0u64), fp!(5u64)]);
    println!("P₂(x) = {}", p2);

    // P(x) = x^3 - notice coefficient 1 is omitted for cleaner display
    let p3 = Polynomial::new(vec![fp!(0u64), fp!(0u64), fp!(0u64), fp!(1u64)]);
    println!("P₃(x) = {}", p3);

    // P(x) = x
    let p4 = Polynomial::new(vec![fp!(0u64), fp!(1u64)]);
    println!("P₄(x) = {}", p4);

    // P(x) = 42 (constant polynomial)
    let p5 = Polynomial::new(vec![fp!(42u64)]);
    println!("P₅(x) = {}", p5);
